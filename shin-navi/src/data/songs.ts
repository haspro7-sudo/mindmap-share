// Demo song seed. Titles/artists/years are public metadata; lyrics and audio are never included.
// energy, hypothesis tags, vocal ranges and known rates are demo estimates, not measured data.

export type Genre = 'J-POP' | 'ロック' | 'アニメ' | 'ボカロ' | 'アイドル' | 'K-POP' | '洋楽' | '歌謡曲' | '演歌' | 'シティポップ' | 'ヒップホップ' | 'C-POP'

/** Order defines ball longitude sectors and area colour indices. */
export const GENRES: readonly Genre[] = ['J-POP', 'ロック', 'アニメ', 'ボカロ', 'アイドル', 'K-POP', '洋楽', '歌謡曲', '演歌', 'シティポップ', 'ヒップホップ', 'C-POP']

export type Vibe = '盛り上がる' | 'しっとり' | 'エモい' | 'みんなで' | 'ノれる' | '泣ける' | '叫べる' | '懐かしい' | 'かっこいい' | 'かわいい' | 'デュエット' | 'ラスト向き' | '1曲目向き'

export type Lang = 'ja' | 'en' | 'ko' | 'zh'
export type Generation = 10 | 20 | 30 | 40 | 50
export type VersionId = 'original' | 'artistMv' | 'anime' | 'karaoke'
export type Tempo = 'slow' | 'mid' | 'fast'

export type EvidenceTag =
  | { kind: 'genre'; value: Genre }
  | { kind: 'decade'; value: `${number}s` }
  | { kind: 'tempo'; value: Tempo }
  | { kind: 'lang'; value: Lang }
  | { kind: 'version'; value: VersionId }

export type InboundTitle = { romaji?: string; en?: string; zhHant?: string; zhHans?: string; ko?: string }
...
export type Song = {
  ...
  genre: Genre
  bpm: number
  /** bpm < 95 slow / <= 135 mid / otherwise fast */
  tempo: Tempo
  /** evidence tags (solid) and hypothesis tags (dotted, "ナビの見立て") */
  tags: { evidence: EvidenceTag[]; hypothesis: Vibe[] }
  /** 0..1, how much the song lifts a room (hypothesis) */
  energy: number
  /** comfortable range estimate as MIDI note numbers (C4 = 60), demo estimate */
  vocalRange: { low: number; high: number }
  /** share of each generation assumed to know the song, 0..100 (demo estimate) */
  knownRate: Record<Generation, number>
  lang: Lang
  /** local-script titles, only where confident */
  inboundTitle: InboundTitle
  /** "一緒に選ばれている" song ids */
  coOccurrence: string[]
  coSource: 'curated' | 'computed'
  versions: VersionId[]
  reservable: boolean
  trending?: boolean
}
...
const tempoOf = (bpm: number): Tempo => (bpm < 95 ? 'slow' : bpm <= 135 ? 'mid' : 'fast')

export const decadeOf = (s: { year: number }): `${number}s` => `${Math.floor(s.year / 10) * 10}s`

const CURATED_CO: Record<string, string[]> = {
  marigold: ['kimi-rock', 'suiheisen', 'dry-flower', 'que-sera', 'hakujitsu'],
  gurenge: ['homura', 'zankyou-sanka', 'shinjidai', 'kaikai-kitan', 'ao-no-sumika'],
  zankoku: ['tamashii-refrain', 'cha-la', 'only-my-railgun', 'guren-no-yumiya', 'moonlight-densetsu'],
  idol: ['shinjidai', 'kick-back', 'bbbb', 'ao-no-sumika', 'que-sera'],
  lemon: ['uchiage-hanabi', 'pretender', 'hakujitsu', 'dry-flower', 'kanden'],
  'plastic-love': ['mayonaka-no-door', 'ruby-no-yubiwa', 'marunouchi', 'first-love', 'koi'],
  'first-love': ['hanataba', 'can-you-celebrate', 'automatic', 'itoh', 'hanamizuki'],
  'que-sera': ['lilac', 'ao-to-natsu', 'marigold', 'mixed-nuts', 'kaiju-hanauta'],
  'yoru-ni-kakeru': ['gunjou', 'idol', 'kaiju-hanauta', 'kanden', 'zenzenzense'],
  pretender: ['subtitle', 'mixed-nuts', 'hakujitsu', 'lemon', 'cinderella-boy'],
}

const NOT_RESERVABLE = new Set(['magnetic', 'xiao-xing-yun'])
const TRENDING = new Set(['bbbb', 'que-sera', 'lilac', 'otonoke', 'bansanka', 'idol'])
const MV_GENRES = new Set<Genre>(['J-POP', 'ロック', 'アイドル', 'K-POP'])

function versionsOf(genre: Genre, year: number): VersionId[] {
  const v: VersionId[] = ['original']
  if (MV_GENRES.has(genre) && year >= 2000) v.push('artistMv')
  if (genre === 'アニメ') v.push('anime')
  v.push('karaoke')
  return v
}

type Base = Omit<Song, 'coOccurrence' | 'coSource'>

const BASE: Base[] = ROWS.map(([id, title, artist, year, genre, bpm, energy, vibes, range, k, lang = 'ja', titles = {}]) => {
  const tempo = tempoOf(bpm)
  const versions = versionsOf(genre, year)
  return {
    id,
    title,
    artist,
    ...
    tags: {
      evidence: [
        { kind: 'genre', value: genre },
        { kind: 'decade', value: decadeOf({ year }) },
        { kind: 'tempo', value: tempo },
        { kind: 'lang', value: lang },
        ...versions.filter(v => v !== 'original' && v !== 'karaoke').map(v => ({ kind: 'version' as const, value: v })),
      ],
      hypothesis: vibes,
    },
    energy,
    vocalRange: { low: range[0], high: range[1] },
    knownRate: { 10: k[0], 20: k[1], 30: k[2], 40: k[3], 50: k[4] },
    lang,
    inboundTitle: titles,
    versions,
    reservable: !NOT_RESERVABLE.has(id),
    ...(TRENDING.has(id) ? { trending: true } : {}),
  }
})

/** Similarity used for computed co-occurrence (era, genre, vibes, artist, energy). */
function similarity(a: Base, b: Base): number {
  const vibeOverlap = b.tags.hypothesis.filter(v => a.tags.hypothesis.includes(v)).length
  const era = 1 - Math.min(1, Math.abs(b.year - a.year) / 25)
  const g = b.genre === a.genre ? 1 : 0
  const artist = b.artist === a.artist ? 0.6 : 0
  return vibeOverlap * 0.5 + era * 1.2 + g * 0.8 + artist + (1 - Math.abs(b.energy - a.energy)) * 0.6
}

export const SONGS: Song[] = BASE.map(s => {
  const curated = CURATED_CO[s.id]
  if (curated) return { ...s, coOccurrence: curated, coSource: 'curated' as const }
  const co = BASE.filter(o => o.id !== s.id)
    .map(o => ({ id: o.id, v: similarity(s, o) }))
    .sort((x, y) => y.v - x.v || (x.id < y.id ? -1 : 1))
    .slice(0, 5)
    .map(x => x.id)
  return { ...s, coOccurrence: co, coSource: 'computed' as const }
})
